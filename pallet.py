colors = {
        ...
        "deep-purple": ["ede5f4", "d1c4e9", "b39ddb", "9575cd", "7e57c2", "673ab7", "5e35b1", "512da8", "4527a0", "311b92"],
...
def convert_color(fmt, color):
    color = color.lstrip('#')
    r, g, b = tuple(int(color[i:i+2], 16) for i in (0, 2, 4))
    if fmt == 'fg':
        return "\033[38;2;{};{};{}m##\033[0m".format(r,g,b)
    elif fmt == 'bg':
...
def print_colors():
    for key, val in colors.items():
        for c in val:
            print(convert_color('bg', c), end='')
        print()

def cmap():
    print("std::map<std::string, std::vector<uint32_t>> colors = {")
    for key, val in colors.items():
        print("{{\"{}\", {{{}}}}},".format(key, ','.join(["0x{}".format(x) for x in val])))
    print("};")
